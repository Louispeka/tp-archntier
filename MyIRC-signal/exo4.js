...
// Fonction pour arrêter le serveur avec un avertissement
function shutdownServer(server) {
  console.log("Arrêt du serveur dans 5 secondes...");
  broadcast("Le serveur va fermer dans 5 secondes. Merci de vous reconnecter plus tard.");

  setTimeout(() => {
    broadcast("Le serveur est maintenant hors ligne. À bientôt !");
    clients.forEach((client) => client.socket.end()); // Déconnecter tous les clients
    server.close(() => {
    ...
    if (buffer.includes("\n")) {
      ...
      messages.forEach((message) => {
        ...
        if (message.startsWith("/whisper")) {
          ...
          if (recipient) {
            recipient.socket.write(`[Whisper][${nickname}]: ${whisperMessage}\n\r`);
...
// Gestion des signaux système (SIGINT pour Ctrl+C)
process.on("SIGINT", () => {
